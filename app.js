class TaskManager {
    constructor() {
        ...
        // 加载保存的标签页
        const savedTab = localStorage.getItem('potato_current_tab') || 'tasks';
    ...
    loadNotes() {
        const saved = localStorage.getItem('potato_notes');
        return saved ? JSON.parse(saved) : [];
    }
    
    saveNotes() {
        localStorage.setItem('potato_notes', JSON.stringify(this.notes));
    }
    ...
    initElements() {
        ...
        this.noteCancel = document.getElementById('noteCancel');
        this.noteForm = document.getElementById('noteForm');
        this.noteTitle = document.getElementById('noteTitle');
        this.noteContent = document.getElementById('noteContent');
        this.noteId = document.getElementById('noteId');
        ...
        this.tasksContent = document.getElementById('tasksContent');
    ...
    initEventListeners() {
        ...
        if (this.noteClose) {
            this.noteClose.addEventListener('click', () => this.closeNoteModal());
        }
        
        if (this.noteCancel) {
            this.noteCancel.addEventListener('click', () => this.closeNoteModal());
        }
        
        if (this.noteOverlay) {
            this.noteOverlay.addEventListener('click', (e) => {
                if (e.target === this.noteOverlay) {
                    this.closeNoteModal();
                }
            });
        }
        ...
        // 手账工具栏事件
        this.initNoteEditorToolbar();
        
        // 标签页切换
        if (this.tasksTab) {
            this.tasksTab.addEventListener('click', () => this.switchTab('tasks'));
        }
    ...
    applyBackground(bgType) {
        ...
        // 移除所有背景类
        document.body.classList.remove('bg-default', 'bg-gradient1', 'bg-gradient2', 'bg-gradient3', 'bg-custom', 'bg-aot', 'bg-honkai', 'bg-conan', 'bg-demon-slayer', 'bg-genshin', 'bg-soulstreet');
        
        // 移除自定义背景图片
        const appContainer = document.querySelector('.app-container');
        if (appContainer) {
            appContainer.style.setProperty('--custom-bg', '');
            if (appContainer.querySelector('::before')) {
                appContainer.querySelector('::before').style.backgroundImage = '';
            }
        }
        
        if (bgType === 'custom' && this.customBgImage) {
            document.body.classList.add('bg-custom');
            appContainer.style.setProperty('--custom-bg', `url(${this.customBgImage})`);
        } else if (bgType === 'aot') {
            // 进击的巨人主题背景
            document.body.classList.add('bg-aot');
            appContainer.style.setProperty('--custom-bg', 'url(https://picsum.photos/seed/aot-bg-1920x1080/1920/1080)');
        } else if (bgType === 'honkai') {
            // 崩坏主题背景
            document.body.classList.add('bg-honkai');
            appContainer.style.setProperty('--custom-bg', 'url(https://picsum.photos/seed/honkai-bg-1920x1080/1920/1080)');
        } else if (bgType === 'conan') {
            // 名侦探柯南主题背景
            document.body.classList.add('bg-conan');
            appContainer.style.setProperty('--custom-bg', 'url(https://picsum.photos/seed/conan-bg-1920x1080/1920/1080)');
        } else if (bgType === 'demon-slayer') {
            // 鬼灭之刃主题背景
            document.body.classList.add('bg-demon-slayer');
            appContainer.style.setProperty('--custom-bg', 'url(https://picsum.photos/seed/demonslayer-bg-1920x1080/1920/1080)');
        } else if (bgType === 'genshin') {
            // 原神主题背景
            document.body.classList.add('bg-genshin');
            appContainer.style.setProperty('--custom-bg', 'url(https://picsum.photos/seed/genshin-bg-1920x1080/1920/1080)');
        } else if (bgType === 'soulstreet') {
            // 镇魂街主题背景
            document.body.classList.add('bg-soulstreet');
            appContainer.style.setProperty('--custom-bg', 'url(https://picsum.photos/seed/soulstreet-bg-1920x1080/1920/1080)');
        } else {
...
// 记事本相关方法
TaskManager.prototype.renderNotes = function() {
    if (this.notes.length === 0) {
        this.notesEmptyState.style.display = 'block';
        const existingNotes = this.notesList.querySelectorAll('.note-card');
        ...
        this.notes.forEach(note => {
            fragment.appendChild(this.createNoteElement(note));
        });
        
        this.notesList.innerHTML = '';
        this.notesList.appendChild(this.notesEmptyState);
        this.notesList.appendChild(fragment);
    }
};

TaskManager.prototype.createNoteElement = function(note) {
    const div = document.createElement('div');
    ...
    // 应用保存的样式
    if (note.template) {
        div.classList.add(note.template);
    }
    if (note.highlighter) {
        div.classList.add(note.highlighter);
    }
    if (note.isSticky) {
    ...
    div.innerHTML = `
        ...
        <div class="note-card-date">${new Date(note.createdAt).toLocaleString('zh-CN')}</div>
        <div class="note-card-actions">
            <button class="note-action-btn edit" data-action="edit-note" title="编辑">
                <svg viewBox="0 0 24 24" width="16" height="16">
                    <path fill="currentColor" d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/>
                </svg>
            </button>
            <button class="note-action-btn delete" data-action="delete-note" title="删除">
                <svg viewBox="0 0 24 24" width="16" height="16">
                    <path fill="currentColor" d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/>
                </svg>
            </button>
...
TaskManager.prototype.openNoteModal = function(note = null) {
    this.noteOverlay.classList.add('active');
    document.body.style.overflow = 'hidden';
    
    if (note) {
        this.noteId.value = note.id;
        this.noteModalTitle.textContent = '编辑笔记';
        this.noteTitle.value = note.title;
        this.noteContent.innerHTML = note.content;
        
        // 恢复保存的样式
        this.noteContent.className = '';
        if (note.template) {
            this.noteContent.classList.add(note.template);
        }
        if (note.highlighter) {
            this.noteContent.classList.add(note.highlighter);
        }
        if (note.isSticky) {
            this.noteModal.classList.add('sticky-note');
        } else {
            this.noteModal.classList.remove('sticky-note');
        }
        
        // 加载图片
        this.currentNoteImages = note.images || [];
        this.renderImagePreviews();
    } else {
        this.noteId.value = '';
        this.noteModalTitle.textContent = '新建笔记';
        this.noteForm.reset();
        this.noteContent.innerHTML = '';
        this.noteContent.className = '';
        this.noteModal.classList.remove('sticky-note');
        this.currentNoteImages = [];
...
// 渲染图片预览
TaskManager.prototype.renderImagePreviews = function() {
    const imagePreviewContainer = document.getElementById('imagePreviewContainer');
    if (!imagePreviewContainer) return;
    
    imagePreviewContainer.innerHTML = '';
    
    this.currentNoteImages.forEach((imageData, index) => {
        ...
        previewItem.querySelector('.remove-btn').addEventListener('click', (e) => {
            e.stopPropagation();
            this.currentNoteImages.splice(index, 1);
            previewItem.remove();
        ...
        previewItem.addEventListener('click', () => {
            const imgTag = `<img src="${imageData}" style="max-width: 100%; border-radius: 8px; margin: 8px 0;">`;
            const startPos = this.noteContent.selectionStart || 0;
            const endPos = this.noteContent.selectionEnd || 0;
            const currentValue = this.noteContent.innerHTML;
            this.noteContent.innerHTML = currentValue.substring(0, startPos) + imgTag + currentValue.substring(endPos);
            this.hasUnsavedChanges = true;
        });
...
TaskManager.prototype.closeNoteModal = function() {
    this.noteOverlay.classList.remove('active');
    document.body.style.overflow = '';
};

// 标签页切换方法
TaskManager.prototype.switchTab = function(tab) {
    // 更新标签页按钮状态
    this.tasksTab.classList.toggle('active', tab === 'tasks');
    this.notesTab.classList.toggle('active', tab === 'notes');
    
    // 更新内容显示
    this.tasksContent.classList.toggle('active', tab === 'tasks');
    this.notesContent.classList.toggle('active', tab === 'notes');
    
    // 保存当前标签页
    localStorage.setItem('potato_current_tab', tab);
};

// 添加任务按钮显示控制
TaskManager.prototype.updateAddTaskButton = function() {
    // 只有在任务标签页才显示添加任务按钮
    const isTasksTab = this.tasksContent.classList.contains('active');
    if (this.addTaskBtn) {
        this.addTaskBtn.style.display = isTasksTab ? 'flex' : 'none';
    }
};

TaskManager.prototype.deleteNote = function(id) {
...
TaskManager.prototype.viewNote = function(note) {
    // 创建查看笔记的弹窗
    const viewModal = document.createElement('div');
    viewModal.className = 'modal-overlay';
    viewModal.style.cssText = 'position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; z-index: 10000;';
    
    // 构建样式类
    let contentClasses = '';
    if (note.template) contentClasses += ` ${note.template}`;
    if (note.highlighter) contentClasses += ` ${note.highlighter}`;
    
    viewModal.innerHTML = `
        <div class="modal" style="max-width: 600px; background: var(--bg-secondary); border-radius: var(--radius-xl); padding: 30px; position: relative;">
            <h2 style="font-size: 20px; margin-bottom: 16px; color: var(--text-primary);">${this.escapeHtml(note.title)}</h2>
            <div style="font-size: 14px; color: var(--text-muted); margin-bottom: 20px;">${new Date(note.createdAt).toLocaleString('zh-CN')}</div>
            <div class="${contentClasses.trim()}" style="white-space: pre-wrap; line-height: 1.8; color: var(--text-secondary); margin-bottom: 30px; max-height: 400px; overflow-y: auto;">${this.escapeHtml(note.content)}</div>
            <button class="submit-btn" style="width: 100%;">关闭</button>
        </div>
    `;
    
    document.body.appendChild(viewModal);
    
    viewModal.querySelector('.submit-btn').addEventListener('click', () => {
        viewModal.remove();
    });
...
// 手账工具栏初始化
TaskManager.prototype.initNoteEditorToolbar = function() {
    const toolbarBtns = document.querySelectorAll('.note-editor-btn');
    const templateSelector = document.getElementById('templateSelector');
    const stickerSelector = document.getElementById('stickerSelector');
    const highlighterSelector = document.getElementById('highlighterSelector');
    ...
    // 存储当前笔记的图片和荧光笔标记
    this.currentNoteImages = [];
    this.currentHighlighterColor = null;
    ...
    toolbarBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (action === 'template') {
                const isVisible = templateSelector.style.display === 'block';
                templateSelector.style.display = isVisible ? 'none' : 'block';
                stickerSelector.style.display = 'none';
                highlighterSelector.style.display = 'none';
                imageUploader.style.display = 'none';
                
                // 切换按钮激活状态
                toolbarBtns.forEach(b => {
                    if (b.dataset.action === 'template') {
                ...
                const isVisible = stickerSelector.style.display === 'block';
                stickerSelector.style.display = isVisible ? 'none' : 'block';
                ...
                const isVisible = highlighterSelector.style.display === 'block';
                highlighterSelector.style.display = isVisible ? 'block' : 'none';
                templateSelector.style.display = 'none';
                stickerSelector.style.display = 'none';
                imageUploader.style.display = 'none';
                ...
                // 切换按钮激活状态
                toolbarBtns.forEach(b => {
                    if (b.dataset.action === 'highlighter') {
                        b.classList.toggle('active', !isVisible);
                    } else {
                        b.classList.remove('active');
                    }
                ...
                // 如果打开荧光笔，提示选择颜色
                if (!isVisible) {
                    this.isHighlighterActive = true;
                } else {
                    this.isHighlighterActive = false;
                }
            } else if (action === 'image') {
                const isVisible = imageUploader.style.display === 'block';
                imageUploader.style.display = isVisible ? 'none' : 'block';
                templateSelector.style.display = 'none';
                stickerSelector.style.display = 'none';
                highlighterSelector.style.display = 'none';
                ...
                // 便签纸效果
                this.noteModal.classList.toggle('sticky-note');
    ...
    // 荧光笔颜色选择
    const highlighterOptions = document.querySelectorAll('.highlighter-option');
    highlighterOptions.forEach(option => {
        option.addEventListener('click', () => {
            highlighterOptions.forEach(o => o.classList.remove('selected'));
            ...
            // 提示用户选择文本
            const tips = highlighterSelector.querySelector('.highlighter-tips p');
            tips.textContent = '✏️ 现在请在文本中选择要涂抹的文字！';
            ...
            setTimeout(() => {
                highlighterSelector.style.display = 'none';
                toolbarBtns.forEach(btn => {
                    if (btn.dataset.action === 'highlighter') {
    ...
    // 监听文本选择，实现荧光笔功能
    this.noteContent.addEventListener('mouseup', () => {
        if (this.isHighlighterActive && this.currentHighlighterColor) {
            const selection = window.getSelection();
            if (selection.toString().trim().length > 0) {
                this.applyHighlighter(selection, this.currentHighlighterColor);
            }
        }
    });
    ...
    templateOptions.forEach(option => {
        option.addEventListener('click', () => {
            ...
            this.noteContent.className = this.noteContent.className.replace(/template-\w+/g, '');
            
            if (template) {
                this.noteContent.classList.add(`template-${template}`);
            }
        });
    });
    ...
    stickerOptions.forEach(option => {
        option.addEventListener('click', () => {
            ...
            // 在内容开头添加贴纸
            const currentValue = this.noteContent.innerHTML;
            this.noteContent.innerHTML = sticker + ' ' + currentValue;
            
            // 自动关闭贴纸选择器
            setTimeout(() => {
                stickerSelector.style.display = 'none';
                toolbarBtns.forEach(btn => {
                    if (btn.dataset.action === 'sticker') {
                        btn.classList.remove('active');
                    }
                });
            }, 300);
...
// 应用荧光笔高亮
TaskManager.prototype.applyHighlighter = function(selection, color) {
    try {
        const range = selection.getRangeAt(0);
        const span = document.createElement('span');
        span.className = `highlighter-mark ${color}`;
        ...
        // 恢复提示文字
        const tips = document.querySelector('.highlighter-tips p');
        if (tips) {
            tips.textContent = '💡 提示：选择颜色后，在文本区域涂抹即可高亮';
        }
    } catch (e) {
        console.error('荧光笔应用失败:', e);
    }
};

// 处理图片文件
TaskManager.prototype.handleImageFiles = function(files) {
    const imagePreviewContainer = document.getElementById('imagePreviewContainer');
    ...
    Array.from(files).forEach(file => {
        ...
        reader.onload = (e) => {
            ...
            this.currentNoteImages.push(imageData);
            ...
            previewItem.innerHTML = `
                <img src="${imageData}" alt="预览图片">
                <button class="remove-btn" data-index="${this.currentNoteImages.length - 1}">×</button>
            `;
            ...
            previewItem.addEventListener('click', () => {
                ...
                textarea.value = currentValue.substring(0, startPos) + imgTag + currentValue.substring(endPos);
...
// 更新图片预览索引
TaskManager.prototype.updateImagePreviews = function() {
...
// 保存笔记时保存装饰信息
TaskManager.prototype.handleNoteSubmit = function(e) {
    e.preventDefault();
    
    const noteData = {
        title: this.noteTitle.value.trim(),
        content: this.noteContent.innerHTML.trim(),
        template: this.noteContent.className.match(/template-\w+/)?.[0] || '',
        highlighter: this.noteContent.className.match(/highlighter-\w+/)?.[0] || '',
        isSticky: this.noteModal.classList.contains('sticky-note'),
        images: this.currentNoteImages || []
    };
    
    const noteId = this.noteId.value;
    ...
    if (noteId) {
        ...
        if (note) {
            ...
            note.template = noteData.template;
            note.highlighter = noteData.highlighter;
            note.isSticky = noteData.isSticky;
        ...
        const note = {
            ...
            highlighter: noteData.highlighter,
            isSticky: noteData.isSticky,
    ...
    // 更新原始内容（防止保存后再次提示）
    this.originalNoteTitle = noteData.title;
    this.originalNoteContent = noteData.content;
